...
namespace CWM.Skinn
{
    public partial class VertexMapperInspector
    {
        private class ShapeTreeGUI
        {
            ...
            const string kSessionStateKeyPrefix = "ShapeTree325326";
            ...
            public void OnGUI(Rect position, VertexMapper vm, string stateHash = "")
            {
                ...
                EditorGUI.LabelField(position.SetHeight(labelHeight).OffsetX(30)
                    , "Shapes: " + SkinnEx.EnforceObjectName(skinnedMesh), EditorStyles.miniLabel);

                if (!skinnedMesh) { return; }

                filter = vm.boneFilter;

                if (!filter.IsValidShapes(skinnedMesh))
                {
                    filter.GenerateAsShapes(skinnedMesh);
            ...
            private void OnGizmoSelectionChanged()
            {
                ...
                //IList<int> selection = handles.selection;
            ...
            private void OnTreeSelectionChanged()
            {
                ...
                //handles.selection = new List<int>(treeview.GetSelection());
            }

            private void OnBeforeDroppingDraggedItems(IList<TreeViewItem> draggedRows)
            {
                if (!treeview || !filter || !vm) return;
                Undo.RegisterCompleteObjectUndo(vm, string.Format("{0} Hierarchy Adjusted", SkinnEx.EnforceString(filter.name)));
