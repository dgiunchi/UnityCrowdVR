...
public class NavmeshController {
	...
	public Vector3 QueryMove() {
		...
		move.z += AnimationInputHandlerFromNavmesh.GetKey(Forward);
		move.z += AnimationInputHandlerFromNavmesh.GetKey(Back);
		move.x += AnimationInputHandlerFromNavmesh.GetKey(Left);
		move.x += AnimationInputHandlerFromNavmesh.GetKey(Right);
	...
	public float QueryTurn() {
		float turn = 0f;
        turn += AnimationInputHandlerFromNavmesh.GetKey(TurnLeft);
	    turn += AnimationInputHandlerFromNavmesh.GetKey(TurnRight);
	...
	public float PoolBias(float[] weights) {
		...
		for(int i=0; i<weights.Length; i++) {
			...
			for(int j=0; j<Styles[i].Multipliers.Length; j++) {
				if(AnimationInputHandlerFromNavmesh.GetKey(Styles[i].Multipliers[j].Key) != 0.0f) {
			...
			for(int i=0; i<Keys.Length; i++) {
				if(!Negations[i]) {
					if(Keys[i] == KeyCode.None) {
						if(!AnimationInputHandlerFromNavmesh.anyKey) {
							active = true;
						}
					} else {
						if(AnimationInputHandlerFromNavmesh.GetKey(Keys[i]) != 0.0f ) {
							active = true;
						}
					}
				}
