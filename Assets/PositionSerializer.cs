...
public class PositionSerializer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (SimulationManager.status == SimulationManager.STATUS.PLAY)
        {
            if (fileLoaded)
            {
                if(SimulationManager.Instance.singlePlay)
                {
                    ReadSingleDataFromSimulationPerFrame(SimulationManager.Instance.indexPlay);
                } else
                {
            ...
            for (int j = 0; j < skeletonsList.Count; j++)
            {
                ...
                if(frameIndexPerSkeleton[j] < component1.timedPositions.Count && Math.Abs(component1.timedPositions[frameIndexPerSkeleton[j]].time - t) < 0.01f ) 
                {
                    for (int k = 0; k < jointsNumbers; k++)
                    {
                        ...
                        count += (PositionSerializer.timeAndIndex + PositionSerializer.positionCoord);
                    }
    ...
    public void ReadDataFromSimulationPerFrame() // rewrite the function with a different cumulative data that take in account the timeframe
    {
        ...
        {
            GameObject parent = skeletonJoints[s][0].gameObject;
